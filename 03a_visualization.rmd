...
load("/work/tfs3/gsAI/4paul/CV_noQCAllGens.RData")
## these correlations are for run done on 10/1. w/o QC, all gens.

ml_models_path <- "/work/tfs3/gsAI/gebvs/sep22_allhpt_noqc_GEBVs_10foldCV.csv"
r_models_path <- "/work/tfs3/gsAI/4paul/Oct01_CrossValDarpaGebv_noQC.xlsx"
ml_metrics_path <- "/work/tfs3/gsAI/gebvs/sep22_allhpt_noqc_fold_metrics.csv"
r_metrics_path <- "/work/tfs3/gsAI/4paul/Oct01_CrossValDarpaFoldMetrics.csv"

DARPAGEBV <- read.csv(ml_models_path) 
DARPAGEBV2 <- read_xlsx(r_models_path) 
df <- merge(DARPAGEBV, DARPAGEBV2)

## Turn on to overwrite ML GEBV CSV with R MODEL VALUES
...
RUN THIS CODE CHUNK FOR 1 GEN DATA and FILTER OUT EXTRA ROWS.
```{r}

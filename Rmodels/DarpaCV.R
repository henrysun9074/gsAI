# Example usage
# Rscript DarpaCV.R \
  # "phenTrainDarpa.xlsx" \   ## file with phenotype data
  # "/work/tfs3/gsAI/MAF01_DarpaQCFiltered.csv" \ ## file with genotype data
  # "allMAF01QC" \ ## prefix for output directory to save outputs in
  # 0.2   ## size of evaluation set, we never change this from 0.2 for consistency
...
for (i in 1:10) {
  ...
  # Metrics of model performance to record
  metrics_list <- list()
  ...
  # 5-fold cross validation with logging of each fold 
  for (j in 1:5) {
    cat(sprintf("-- Fold %d --\n", j))
...
# Save fold metrics in gebvs folder
...
# Create dataframe with GEBVs from all models
DARPAGEBV <- cbind(
...
## Save GEBVs also in gebvs directory

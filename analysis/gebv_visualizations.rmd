...
cor_df <- data.frame(
  ...
  Correlation = c(
    ...
    cor(DARPAGEBV$LR, DARPAGEBV$Status, use = "complete.obs"),
    cor(DARPAGEBV$RF,    DARPAGEBV$Status, use = "complete.obs"),
    cor(DARPAGEBV$GB, DARPAGEBV$Status, use = "complete.obs")
  )
)
...
```{r}
gebv_cols <- c("BayesB", "BRR", "EGBLUP", "GBLUP",
               "LASSO", "RKHS", "GB", "LR", "RF")
df_long <- df %>%
...
all_models <- unique(as.character(df_long$Model))
other_models <- setdiff(all_models, priority_models)
new_model_order <- c(priority_models, other_models)
df_long$Model <- factor(df_long$Model, levels = new_model_order)
